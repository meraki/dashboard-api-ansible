...
class NetworksApplianceTrafficShapingCustomPerformanceClasses(object):
    ...
    def exists(self):
        ...
        if name_exists:
            _id = prev_obj.get("id")
            _id = _id or prev_obj.get("customPerformanceClassId")
            if id_exists and name_exists and o_id != _id:
                raise InconsistentParameters(
                    "The 'id' and 'name' params don't refer to the same object")
            if _id:
                self.new_object.update(dict(id=_id))
                self.new_object.update(dict(customPerformanceClassId=_id))
            if _id:
