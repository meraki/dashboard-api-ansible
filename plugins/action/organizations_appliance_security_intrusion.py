...
class OrganizationsApplianceSecurityIntrusion(object):
    ...
    def update_all_params(self):
        new_object_params = {}
        if self.new_object.get('allowedRules') is not None or self.new_object.get(
                'allowed_rules') is not None:
            new_object_params['allowedRules'] = self.new_object.get(
                'allowedRules') or self.new_object.get('allowed_rules')
        if self.new_object.get('organizationId') is not None or self.new_object.get(
                'organization_id') is not None:
            new_object_params['organizationId'] = self.new_object.get(
                'organizationId') or self.new_object.get('organization_id')
