...
class OrganizationsWirelessZigbeeDevices(object):
    ...
    def get_all_params(self, name=None, id=None):
        ...
        if self.new_object.get('isEnrolled') is not None or self.new_object.get(
                'is_enrolled') is not None:
            new_object_params['isEnrolled'] = self.new_object.get(
                'isEnrolled') or self.new_object.get('is_enrolled')
    ...
    def update_by_id_params(self):
        new_object_params = {}
        if self.new_object.get('enrolled') is not None or self.new_object.get(
                'enrolled') is not None:
            new_object_params['enrolled'] = self.new_object.get('enrolled')
        if self.new_object.get('channel') is not None or self.new_object.get(
                'channel') is not None:
            new_object_params['channel'] = self.new_object.get('channel') or \
                self.new_object.get('channel')
        if self.new_object.get('organizationId') is not None or self.new_object.get(
                'organization_id') is not None:
            new_object_params['organizationId'] = self.new_object.get(
                'organizationId') or self.new_object.get('organization_id')
    ...
    def get_object_by_name(self, name):
        ...
        try:
            items = self.meraki.exec_meraki(
                family="wireless",
                function="getOrganizationWirelessZigbeeDevices",
    ...
    def update(self):
        ...
        result = self.meraki.exec_meraki(
            family="wireless",
            function="updateOrganizationWirelessZigbeeDevice",
...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        obj = OrganizationsWirelessZigbeeDevices(self._task.args, meraki)
