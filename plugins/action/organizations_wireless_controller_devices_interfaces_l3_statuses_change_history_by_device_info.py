...
argument_spec.update(dict(
    ...
    includeInterfacesWithoutChanges=dict(type="bool"),
    t0=dict(type="str"),
    t1=dict(type="str"),
...
class ActionModule(ActionBase):
    ...
    def get_all(self, params):
        ...
        if params.get("serials") is not None:
            new_object["serials"] = params.get(
                "serials")
        if params.get("includeInterfacesWithoutChanges") is not None:
            new_object["includeInterfacesWithoutChanges"] = params.get(
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        response = meraki.exec_meraki(
            ...
            function='getOrganizationWirelessControllerDevicesInterfacesL3StatusesChangeHistoryByDevice',
            params=self.get_all(
                self._task.args),
        )
