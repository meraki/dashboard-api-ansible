...
class NetworksApplianceTrafficShapingUplinkBandwidth(object):
    ...
    def update_all_params(self):
        new_object_params = {}
        if self.new_object.get('bandwidthLimits') is not None or self.new_object.get(
                'bandwidth_limits') is not None:
            new_object_params['bandwidthLimits'] = self.new_object.get(
                'bandwidthLimits') or self.new_object.get('bandwidth_limits')
        if self.new_object.get('networkId') is not None or self.new_object.get(
                'network_id') is not None:
            new_object_params['networkId'] = self.new_object.get(
                'networkId') or self.new_object.get('network_id')
