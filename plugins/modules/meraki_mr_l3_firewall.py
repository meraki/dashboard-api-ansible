...
def main():
    ...
    fw_rules = dict(
        policy=dict(
            type='str', choices=[
                'allow', 'deny']), protocol=dict(
            type='str', choices=[
                'tcp', 'udp', 'icmp', 'any']), dest_port=dict(
            type='str'), dest_cidr=dict(
            type='str'), comment=dict(
            type='str'), )
    ...
    if meraki.params['ssid_name']:
        number = get_ssid_number(
            meraki.params['ssid_name'], get_ssids(
                meraki, net_id))
    ...
    if meraki.params['state'] == 'query':
        ...
        try:
            if len(rules) != len(
                    payload['rules']):  # Quick and simple check to avoid more processing
                update = True
            if update is False:
        ...
        if rules['rules'][len(rules['rules']) -
